...
public class RefundableDrinkVendingMachineTest {
	...
	public static void main(String[] args) {
		...
		int orderMoney = machine.pressDrinkButton(1, 3, 10000);	//4500원 음료수 값
		System.out.println("주문금액: " + orderMoney);
		
		if (machine instanceof RefundableDrinkVendingMachine rfMachine) {
			rfMachine.refund();	//5500원 환불
		}
		
		orderMoney = machine.pressDrinkButton(6, 3, 10000);	//6번 제품은 없어서 return 0 받음
		System.out.println("주문금액: " + orderMoney);
		
		if (machine instanceof RefundableDrinkVendingMachine rfMachine) {
			rfMachine.refund();	//넣은돈 그대로 반환
		}
		
		
		DrinkVendingMachine machine2 = new RefundableDrinkVendingMachineUseOverride(박카스, 몬스터, 핫식스, 밀키스);	//오버라이딩 예제 문제 결과
