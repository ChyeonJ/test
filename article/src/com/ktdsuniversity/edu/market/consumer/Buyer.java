...
public class Buyer {
	...
	//private final int PRICE = 1000; => 판매자 소비자 합치기 위함 지움
	//private final int PRODUCT_WEIGHT = 500; => 이것도 판매자가 가지고 있어야할 것
...
//	}
	...
	public void buyFrom(Seller seller, int productCount) {
		...
		// 1. 구매자 : 혹시 두쫀쿠 productCount만큼 있나요?
		...
		if(seller.isEnoughStock(productCount)) {
			...
			if ( this.isEnoughMoney(price * productCount)) {
				...
				if(this.isEnoughWeight(weight * productCount)) {
					...
					// 11.구매자 : 여기 5천원 드릴게요. (구매자의 돈이 감소, 판매자의 자산이 증가, 구매자의 무게와 구매개수 증가)
					this.wallet -= price * productCount;
					int accocunt = seller.getAccount();
					accocunt += price * productCount;
					seller.setAccount(accocunt);
	...
	public void buy(int money, int wehight, int productCount ) {
		if (!this.isEnoughMoney(money)) {
			System.out.println("돈이 부족합니다.");
		} else if (!this.isEnoughWeight(wehight)) {
			System.out.println("더 이상 장바구니를 들 수 없습니다.");
		} else {
			this.wallet -= money;
			this.itemCount += productCount;
			this.nowCartWeight += wehight;
		}
