...
public class DrinkMachineArray {
	...
	DrinkMachine[] drinkM;	//DrinkMachine[] => DrinkMachinea class 배열 선언
	...
	public int orderDrinK(int count, int stock) {
		...
		if(stock == 0 || totalStock >= this.drinkM[count].stock) {
			...
			int totalPrice = this.drinkM[count].price * stock; //상품의 가격 계산
	...
	public void printAllDrink() {
		for(int i = 0; i < this.drinkM.length; i++) {
			System.out.println(this.drinkM[i].drinkName + " " + this.drinkM[i].stock + "개 남았습니다.");
		}
	}
